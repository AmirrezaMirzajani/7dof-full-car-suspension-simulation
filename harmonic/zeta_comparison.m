...
for i=1:(n_RK5)
    ...
    kx1=Zeta_comparison_suspension(t14(i),y14(i,:),zeta_values(j));
    kx2=Zeta_comparison_suspension(t14(i)+0.25*h,y14(i,:)+(1/4)*h*kx1,zeta_values(j));
    kx3=Zeta_comparison_suspension(t14(i)+0.25*h,y14(i,:)+((1/8)*kx1+(1/8)*kx2)*h,zeta_values(j));
    kx4=Zeta_comparison_suspension(t14(i)+0.5*h,y14(i,:)+(-0.5*kx2+kx3)*h,zeta_values(j));
    kx5=Zeta_comparison_suspension(t14(i)+(3/4)*h,y14(i,:)+((3/16)*kx1+(9/16)*kx4)*h,zeta_values(j));
    kx6=Zeta_comparison_suspension(t14(i)+h,y14(i,:)+((-3/7)*kx1+(2/7)*kx2+(12/7)*kx3-(12/7)*kx4+(8/7)*kx5)*h,zeta_values(j));
...
for i=1:n
    Suspention(i,:)=suspension_Zeta(t14(i),phi_dot(i),theta_dot(i),Z_dot(i),zufl_dot(i),zufr_dot(i)...
        ,zurl_dot(i),zurr_dot(i),phi(i),theta(i),zufl(i),zufr(i),zurl(i),zurr(i),Z(i),zeta(j));
end
